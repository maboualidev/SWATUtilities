function output=readSWATBasins(filename)
%% Description:
%  readSWATBasins(filename) receives the path to the SWAT basins file 
%  and parses its contents into a structure.
%
%  input:
%    - filename: path the SWAT basins.bsn
%
%  output:
%    - output: a structure containing different variables defined by
%              SWAT basins.bsn
%
%  NOTE(S):
%    - The fields are based on Chapter 4: SWAT INPUT - .BSN, Version 2012.
%
    %%
    ...
    if (fid==-1)
        error('readSWATBasins: Could not open %s for reading.',filename);
    end
        ...
        for idx=1:size(fieldNames,1)
            if (idx<=lastMandatoryField)
                ...
                switch fieldNames{idx,3}
                    case 'char'
                        if any(strcmpi(fieldNames{idx,1},{'petfile','wwqfile'}))
                            tmpTXT=strsplit(strtrim(tmpTXT),'|');
                            output.(fieldNames{idx,1})=strtrim(tmpTXT{1});
                        else
                            output.(fieldNames{idx,1})=tmpTXT;
                        end
                    case {'integer', 'double'}
                        tmpTXT=strsplit(strtrim(tmpTXT));
                        output.(fieldNames{idx,1})=str2double(tmpTXT{1});
                    case 'doubleArray'
                        output.(fieldNames{idx,1})=str2num(tmpTXT);
                    otherwise
                        error('readSWATBasins.m: does not recognize the type.')
                end
            else
                if (~feof(fid))
                    tmpTXT=fgetl(fid);
                    if (~isempty(strtrim(tmpTXT)))
                        switch fieldNames{idx,3}
                            case 'char'
                                if any(strcmpi(fieldNames{idx,1},{'petfile','wwqfile'}))
                                    tmpTXT=strsplit(strtrim(tmpTXT),'|');
